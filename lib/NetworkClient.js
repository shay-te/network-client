var NetworkClient = function(defaultConfigOverride){
	 ...
	 *
	 **/
    _default_request_config = { baseURL: "", json: false, headers: {}, retries: 0, backOffFactor: 0, store: false, storeExpiration: 0, debug: false};
    _request_config = defaultConfigOverride ? _objectAssign(_default_request_config, defaultConfigOverride) : _default_request_config;
    ...
    function _send(method, url, data, config, success, error) {
        ...
        _ajax(
            ...
            function(responseData) {
                ...
                _fireNetworkEnd(method, fullUrl, data, fullConfig);
                if(fullConfig.store) {
            ...
            function(xhr){
                ...
                _fireNetworkError(method, fullUrl, data, fullConfig);
                if(fullConfig.debug) {
                    _error("ERROR: method:" + method + ", fullUrl:" + fullUrl + ", data:" + _jsonToDataString(data || {}) + ", config:" + _jsonToDataString(fullConfig || {}));
                }
                error(xhr);
   ...
   */
   _headers = {}
    ...
    _setRequestHeaders= function(xhr, method, url, data, config) {
    ...
    _fireNetworkStart= function(method, url, data, config) { _fireEventByKey("networkStart", method, url, data, config); }
    _fireNetworkEnd=   function(method, url, data, config) { _fireEventByKey("networkEnd", method, url, data, config); }
    _fireNetworkError= function(method, url, data, config) { _fireEventByKey("networkError", method, url, data, config); }
    _fireNetworkStorage= function(method, url, data, config) { _fireEventByKey("networkStorage", method, url, data, config); }
    _fireEventByKey= function(eventKey, method, url, data, config) {
        for(var i = 0 ; i < _eventListeners.length ; i++) {
            try { _eventListeners[i][eventKey](method, url, data, config); } catch (e) {_error(e);}
        }
    }
