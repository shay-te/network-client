var NetworkClient = {
    ...
    _default_request_options: { 'json': false, 'headers': {}, 'retries': 0, 'backOffFactor': 0, 'store': false, 'storeExpiration': undefined},
...
//                storeResultsPermanently
     ...
     *
     **/
     _creation_time: this._now(),
     storage: (()) ? localStorage : localStorageFill,
     _build_store_key: function(method, url, data) {
        return method + '-' + url + '-' + this._jsonToDataString(data).toLowerCase();
     },
     _toStorage: function(method, url, data, storeData) {
        this.storage.setItem(this._build_store_key(method, url, data),
                             JSON.stringify({time: this._now(), data: storeData}));
     },
     _fromStorage: function(method, url, data, storeExpiration) {
        var key = this._build_store_key(method, url, data);
        var storeStr = this.storage.getItem(key);
        if(storeStr) {
            var storeItem = JSON.parse(storeStr);
            if(storeExpiration == undefined) { return storeItem.data; } // Never expires
            else if(storeExpiration == 0 && this._creation_time < storeItem.time) { // Expires on reload
                return storeItem.data;
            }
            else if(!isNaN(storeExpiration) && (storeItem.time + storeExpiration) < _now()) { // Expiration of time
                return storeItem.data;
            }
