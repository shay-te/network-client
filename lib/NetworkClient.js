var NetworkClient = function(defaultOptionsOverride){
    ...
    function _send(method, url, data, options, success, error) {
        ...
        if(fullOptions.store) {
            var sorted_result = _fromStorage(method, url, data);
            if(sorted_result) {
                success(sorted_result);
                return;
            }
        }
        _fireEventNetworkStart(method, url, data, fullOptions);
        _ajax(
            ...
            function(responseData) {
                ...
                _fireEventNetworkEnd(method, url, data, fullOptions);
                if(fullOptions.store) {
                    _toStorage(method, url, data, responseData, fullOptions.storeExpiration);
                }
            ...
            function(xhr){
                ...
                _fireEventNetworkError(method, url, data, fullOptions);
                error(xhr);
            }.bind(this)
        );
    }
   ...
   */
   _headers = {}
    ...
    _fireEventNetworkStart= function(method, url, data, options) { _fireEventByKey("networkStart", method, url, data, options); }
    _fireEventNetworkEnd=   function(method, url, data, options) { _fireEventByKey("networkEnd", method, url, data, options); }
    _fireEventNetworkError= function(method, url, data, options) { _fireEventByKey("networkError", method, url, data, options); }
    ...
    **/
    ...
    function _toStorage(method, url, data, storeData, storeExpiration) {
        ...
        _storage.setItem(_build_store_key(method, url, data),
                              JSON.stringify({time: _now(), data: storeData, expires: parseInt(storeExpiration)}));
    }
